    ...
    <div>
        ...
        <el-card>
            ...
            <el-table :data="dataList" border stripe max-height="650">
                ...
                <el-table-column label="设备名" prop="name">
                    <template slot-scope="rowProps">
                        {{deviceMap[rowProps.row.name]}}
                    </template>
                </el-table-column>
                <el-table-column label="数据类型" prop="cmd">
                    <template slot-scope="rowProps">
                        {{cmdMap[rowProps.row.cmd]}}
                    </template>
                </el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            getData(){
                ...
                axios.$get(comm.WEB_URL+'sensorRealList/getsensor',params).then(res => {
                    console.log(res)
                    this.dataList = res;
                    axios.$get(comm.WEB_URL+'sensorRealList/sensorcount',{sensorType:'all'}).then(res => {
                        console.log(res);
            ...
            gotoData(val) {
                this.$router.push({name:'zhijiangRealData', query:{name:val.name, cmd:val.cmd}})
            },
